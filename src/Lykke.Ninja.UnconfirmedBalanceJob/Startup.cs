...
using Lykke.Ninja.UnconfirmedBalanceJob.Binders;
    ...
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            try
            {
                ...
                {
                    options.SwaggerDoc("v1", new Info { Title = "lykke.ninja.unconfirmedbalancejob", Version = "v1" });
        ...
        private static ILog CreateLog(IServiceCollection services, IReloadingManager<GeneralSettings> settings)
        {
            ...
            if (!string.IsNullOrEmpty(dbLogConnectionString) && !(dbLogConnectionString.StartsWith("${") && dbLogConnectionString.EndsWith("}")))
            {
                var persistenceManager = new LykkeLogToAzureStoragePersistenceManager(
                    AzureTableStorage<LogEntity>.Create(dbLogConnectionStringManager, "LykkeNinjaUnconfirmedBalanceJobLogs", consoleLogger),
                    consoleLogger);
